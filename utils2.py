...
def get_pose_by_obd_tf(obd):
  fps = 9.64764
  alpha = 1.0 / (3.6 * fps)
  beta = 0.000901
  gamma = 0.0
  delta = 0.05 * np.pi / 180.0 * -0.0062
  L_car = 2.7
  bias_steering = 0.0
  angle_scale = 540.0 / 35.0
  ...
  # if (theta < 0.000001):
  #   theta = 0.000001
  f1 = lambda: tf.constant(0.000001)
  f2 = lambda: theta
  theta = tf.case([(tf.less(theta, 0.000001), f1)], default=f2)

  phy = 0.0
  tx = -2.0 / theta * tf.sin(theta / 2.0) * s * tf.sin(a + theta / 2.0 + phy)
  tz = 2.0 / theta * tf.sin(theta / 2.0) * s * tf.cos(a + theta / 2.0 + phy)
