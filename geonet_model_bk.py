...
class GeoNetModel(object):
    ...
    def __init__(self, opt, tgt_image, src_image_stack, intrinsics, *argv):
        self.opt = opt
        ...
        if opt.useobd:
            # self.obd = args
            self.obd_warped_image_stack = argv[0]
    ...
    def build_dispnet(self):
        ...
        disp_net = DepthNet()
        self.pred_disp = disp_net(self.dispnet_inputs)
    ...
    def build_posenet(self):
        ...
        # build posenet
        if opt.useobd:
            self.posenet_obd_warped_img_inputs = tf.concat([self.tgt_image, self.obd_warped_image_stack], axis=3)
            pose_net = PoseNet()
            self.pred_poses = pose_net(opt, [self.posenet_inputs, self.posenet_obd_warped_img_inputs])
        else:
            # self.pred_poses = pose_net(opt, self.posenet_inputs, None)
            pose_net = PoseNet()
            self.pred_poses = pose_net(self.posenet_inputs)

    def build_rigid_flow_warping(self):
        opt = self.opt
        bs = opt.batch_size

        # build rigid_bk flow (fwd: tgt->src, bwd: src->tgt)
