        ...
        for line in f.readlines():
            ...
            if float_chars.issuperset(value):
                ...
                try:
                    ...
                    data[key] = np.array(list(map(float, value.split(' ')))) # compatibility of python3
                except ValueError:
...
def generate_depth_map(calib_dir, velo_file_name, im_shape, cam=2, interp=False, vel_depth=False):
    ...
    dupe_inds = [item for item, count in Counter(inds).items() if count > 1] # compatibility of python3
    for dd in dupe_inds:
